...
Calendar.setup = function (params) {
    ...
    function onSelect(cal) {
        ...
        if (cal.params.inputField) {
            ...
            cal.params.inputField.value += cal.printTime();
        }
        if (cal.params.displayArea) {
            cal.params.displayArea.innerHTML = cal.date.print(cal.params.daFormat);
            cal.params.displayArea.innerHTML += cal.printTime();
        }
        if (cal.params.singleClick && cal.dateClicked) {
            cal.callCloseHandler();
        }
        if (cal.params.flatCallback != null && typeof cal.params.flatCallback == "function") {
            cal.params.flatCallback(cal);
        }
    };
    ...
    triggerEl["on" + params.eventName] = function () {
        ...
        if (!params.position) {
            ...
            window.calendar.showAt(params.position[0], params.position[1]);
        }
        return false;
    };
};
